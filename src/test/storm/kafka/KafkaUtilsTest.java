...
public class KafkaUtilsTest {
	...
	public void topicDoesNotExist() throws Exception {
		KafkaUtils.fetchMessages(
				config,
				simpleConsumer,
				new Partition(Broker.fromString(broker
	...
	public void brokerIsDown() throws Exception {
		...
		KafkaUtils.fetchMessages(
				...
				simpleConsumer,
				new Partition(Broker.fromString(broker
						.getBrokerConnectionString()), 0), OffsetRequest
						.LatestTime());
	}
	...
	public void fetchMessage() throws Exception {
		...
		ByteBufferMessageSet messageAndOffsets = KafkaUtils.fetchMessages(
				...
				simpleConsumer,
				new Partition(Broker.fromString(broker
						.getBrokerConnectionString()), 0), offset);
	...
	public void fetchMessagesWithInvalidOffsetAndDefaultHandlingDisabled()
		...
		KafkaUtils.fetchMessages(
				...
				simpleConsumer,
				new Partition(Broker.fromString(broker
						.getBrokerConnectionString()), 0), -99);
	}
	...
	public void generateTuplesWithoutKeyAndKeyValueScheme() {
		config.scheme = new KeyValueSchemeAsMultiScheme(
				new StringKeyValueScheme());
	...
	public void generateTuplesWithKeyAndKeyValueScheme() {
		...
		for (MessageAndOffset msg : messageAndOffsets) {
			Iterable<List<Object>> lists = KafkaUtils.generateTuples(config,
					msg.message());
			assertEquals(ImmutableMap.of(key, value), lists.iterator().next()
					.get(0));
		}
	}
	...
	private ByteBufferMessageSet getLastMessage() {
		...
		return KafkaUtils.fetchMessages(config, simpleConsumer, new Partition(
				Broker.fromString(broker.getBrokerConnectionString()), 0),
				offsetOfLastMessage);
	}
