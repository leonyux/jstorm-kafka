...
public class ZkCoordinatorLocalBatch implements PartitionCoordinatorBatch {
	public static final Logger LOG = LoggerFactory
			.getLogger(ZkCoordinatorLocalBatch.class);

	SpoutConfig _spoutConfig;
	int _taskIndex;
	int _totalTasks;
	String _topologyInstanceId;
	Map<Partition, PartitionManagerBatch> _managers = new HashMap();
	List<PartitionManagerBatch> _cachedList;
	...
	public ZkCoordinatorLocalBatch(DynamicPartitionConnections connections,
	...
	// 返回定时刷新的PartitionManagerBatch
	@Override
	public List<PartitionManagerBatch> getMyManagedPartitions() {
	...
	void refresh() {
		try {
			...
			synchronized (_managers) {// 锁定_manager结构，在jstorm下支持nextTuple和ack、fail的并发
				...
				for (Partition id : deletedPartitions) {
					PartitionManagerBatch man = _managers.remove(id);
					man.close();
				}
				...
				for (Partition id : newPartitions) {
					PartitionManagerBatch man = new PartitionManagerBatch(_connections,
		...
		_cachedList = new ArrayList<PartitionManagerBatch>(_managers.values());
		LOG.info("Finished refreshing");
	}

	@Override
	public PartitionManagerBatch getManager(Partition partition) {
		synchronized (_managers) {// 锁定_manager结构，在jstorm下支持nextTuple和ack、fail的并发
