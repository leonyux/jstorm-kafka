...
public class PartitionManagerBatchMod implements PartitionManager {
    public static final Logger LOG = LoggerFactory
            .getLogger(PartitionManagerBatchMod.class);
    ...
    LinkedList<KafkaTupleBatch> _waitingToEmit = new LinkedList<KafkaTupleBatch>();
    ...
    public static class KafkaTupleBatch {
        public long _startOffset;
        public List<List<Object>> _tuples;
        public KafkaTupleBatch(long startOffset, List<List<Object>> tuples) {
            _startOffset = startOffset;
            _tuples = tuples;
        }
    }
    public PartitionManagerBatchMod(DynamicPartitionConnections connections,
    ...
    public EmitState next(SpoutOutputCollector collector) {
        ...
        while (true) {
            KafkaTupleBatch batchToEmit = _waitingToEmit.pollFirst();
            if (batchToEmit == null) {
                return EmitState.NO_EMITTED;
            }
            long startOffset = batchToEmit._startOffset;            
            if (!batchToEmit._tuples.isEmpty()) {
                LOG.debug("Emit batch from offset: " + startOffset + " with "
                        + batchToEmit._tuples.size() + " tups");
                collector.emit(new Values(batchToEmit._tuples), new KafkaMessageId(
    ...
    private void fill() {
        ...
        synchronized (_emittedToOffsetLock) {// 多线程下_pending和_emittedToOffset一致
            ...
            List<List<Object>> batchList = null;
            long startOffset = _emittedToOffset;
            for (MessageAndOffset msg : msgs) {
                long offset = msg.offset();
                if (offset != _emittedToOffset){
                    LOG.warn("Offset doesn't match: _emmittedTo=" + _emittedToOffset + " msg.offset=" + offset);
                }
                if (batchCount == 0) {
                    batchList = new ArrayList<List<Object>>(_batchSize);
                } 
                // 对于kafka中的每条消息可以生成多个tuples
                // 输出tuple，一个offset中的多条消息公用一个MessageId？
                Iterable<List<Object>> tups = KafkaUtils.generateTuples(
                        _spoutConfig, msg.message());
                batchList.addAll((List<List<Object>>) tups);  
                batchCount++;
                _emittedToOffset = msg.nextOffset();
                if (batchCount >= _batchSize) {
                    _waitingToEmit.add(new KafkaTupleBatch(startOffset, batchList));
                    batchCount = 0;
                    batchList = null;
