...
public class KafkaUtils {
    ...
    public static IBrokerReader makeBrokerReader(Map stormConf, KafkaConfig conf) {
        if (conf.hosts instanceof StaticHosts) {
            ...
            return new StaticBrokerReader(
                    ((StaticHosts) conf.hosts).getPartitionInformation());
        } else {
            // ZkBrokerReader从Zk中获取Broker信息
            return new ZkBrokerReader(stormConf, conf.topic,
                    (ZkHosts) conf.hosts);
        }
    }
    ...
    // 获取当前起始offset，用到了一些kafka
    // api，参考https://cwiki.apache.org/confluence/display/KAFKA/0.8.0+SimpleConsumer+Example
    public static long getOffset(SimpleConsumer consumer, String topic,
    ...
    public static class KafkaOffsetMetric implements IMetric {
        ...
        public KafkaOffsetMetric(String topic,
        ...
        public Object getValueAndReset() {
            try {
                ...
                if (_partitions != null
                    ...
                    for (Map.Entry<Partition, Long> e : _partitionToOffset
                            .entrySet()) {
                        Partition partition = e.getKey();
                        SimpleConsumer consumer = _connections
                                .getConnection(partition);
                        if (consumer == null) {
                            LOG.warn("partitionToOffset contains partition not found in _connections. Stale partition data?");
                            return null;
                        }
                        long latestTimeOffset = getOffset(consumer, _topic,
                                partition.partition,
                        ...
                        ret.put(partition.getId() + "/" + "latestTime",
                                latestTimeOffset);
                        ret.put(partition.getId() + "/" + "latestEmittedOffset",
                                latestEmittedOffset);
                ...
                LOG.warn(
                        "Metrics Tick: Exception when computing kafkaOffset metric.",
                        t);
            }
            return null;
        }
    ...
    public static Iterable<List<Object>> generateTuples(
